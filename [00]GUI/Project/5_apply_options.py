...
def merge_image():
    ...
    # 가로 넓이
        ...
        if img_width == "원본유지":
            img_width = -1  # 원본유지는 숫자가 아니다 -1 일때는 원본 기준
        else:
        ...
        # 계산식
        # 100 * 60 이미지  width를 80으로 줄이면 height 는?
        ...
        for idx, img in enumerate(images):
            # width 가 원본이 아닐 때에는 사이즈 조정을 해야한다.
            if img_width > -1:
                img = img.resize(image_sizes[idx])

            result_img.paste(img, (0, y_offset))
            y_offset += (img.size[1] + img_space)   # height 값 + 사용자가 지정한 값
        ...
        #  포맷 옵션 처리
        file_name = "mitier_photo." + img_format
